...
def upload_to_vk(video_path, description="", title=""):
    ...
    try:
        ...
        # Prepare message
        message = description if description else "HISTOIRE DES FEMMES ANCIENNES"

        # Ensure message is not empty (VK requirement)
        if not message.strip():
            message = "HISTOIRE DES FEMMES ANCIENNES"
        ...
        video = upload.video(
            ...
            name=title or 'HISTOIRE DES FEMMES ANCIENNES',
...
def main():
    ...
    title = story_en[:100] if story_en else (topic if topic else "Ancient Women's History")
